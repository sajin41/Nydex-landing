...
export const Register = () => {
  ...
  const handleRegister = async (e) => {
    ...
    try {
      ...
      setError(err.response?.data?.error || 'Registration failed. Email or Username might be taken.');
    }
  };

  return (
    <div className="min-h-[60vh] flex items-center justify-center">
      <div className="bg-card p-8 rounded-2xl border border-border w-full max-w-md shadow-xl relative overflow-hidden">
        <div className="animate-in fade-in slide-in-from-bottom-4 duration-300">
          <h2 className="text-3xl font-bold mb-6 text-center">Register</h2>
          {error && <p className="text-destructive text-sm bg-destructive/10 p-3 rounded-lg mb-4">{error}</p>}
          {success && <p className="text-green-500 text-sm bg-green-500/10 p-3 rounded-lg mb-4 text-center">{success}</p>}
